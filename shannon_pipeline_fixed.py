#!/usr/bin/env python3
"""
Fiesta Agency Shannon Verification Orchestrator (Fixed)
--------------------------------------------------------
Implements zero-trust, stateless validation pipeline for Shannon minting.
Fixed version with corrected syntax and improved error handling.

Intended to run as a stateless container/function behind an HTTP gateway.
"""
...
def rate_limit_allowed(client_ip: str) -> bool:
    """Return True if request from client_ip is within rate limit."""
    now = time.time()
    with _rate_lock:
...
def validate_trigger(trigger: str) -> Tuple[bool, str]:
    """Validate trigger field matches expected value."""
...
def validate_context_hash(chash: str) -> Tuple[bool, str]:
    """Validate context_hash matches expected hash."""
...
def validate_attestation(attest: str) -> Tuple[bool, str]:
    """Validate attestation matches exactly."""
...
def validate_timestamp(ts_str: str) -> Tuple[bool, str]:
    """Validate ISO-8601 timestamp within allowed skew."""
...
def write_validation_log(client_ip: str, action: str, result: str, error_code: str = "") -> None:
    ...
    try:
        ...
        sys.stderr.write(f"Failed to write validation log: {e}\n")

# ---- Main orchestration ----
def handle_shannon_request(payload: Dict[str, Any], client_ip: str = "unknown") -> Dict[str, Any]:
    """Process a Shannon minting request."""
    # 1. Size check
    try:
    ...
    # 3. Field presence & type safety
    required = {"action", "trigger", "context_hash", "attestation", "agent", "timestamp"}
    ...
    # Use stripped attestation for hashing (without optional trailing '\n')
