...
path = pathlib.Path('/root/.openclaw/workspace/article-happy-birthday-godson.md')
lines = [l for l in path.read_text().split('\n') if l.strip()]
title = lines[0].lstrip('#').strip()
body = '\n'.join(lines[1:]).strip()
r = gql('mutation P($i:PublishPostInput!){publishPost(input:$i){post{url}}}',{'i':{'publicationId':pub_id,'title':title,'contentMarkdown':body,'tags':[]}})
if 'errors' in r:
    print('ERR',r['errors'])
else:
