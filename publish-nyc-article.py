...
path = pathlib.Path('/root/.openclaw/workspace/article-nyc-survival-updated.md')
lines = [l for l in path.read_text().split('\n') if l.strip()]
title = lines[0].lstrip('#').strip()
body = '\n'.join(lines[1:]).strip()
r = gql('mutation P($i:PublishPostInput!){publishPost(input:$i){post{url}}}',{'i':{'publicationId':pub_id,'title':title,'contentMarkdown':body,'tags':[{'slug':'nyc','name':'NYC'},{'slug':'life','name':'Life'},{'slug':'survival','name':'Survival'},{'slug':'humor','name':'Humor'}]}})
if 'errors' in r:
    print('ERR',r['errors'])
else:
